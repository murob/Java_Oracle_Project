...
실행해야한다. 사용자 계정 생성후 접속 테스트는 CMD(명령프롬프트)에서
진행하도록 한다.
*/
...
형식]
    grant 시스템권한1[,시스템권한2 ....] | [역할1[,역할2....]]
        ...
        [with grant option];
*/
--접속 권한 부여 : 접속은 되지만 테이블은 생성할 수 없다.
...
테이블 스페이스란?
    ...
    hr계정의 데이터를 저장하는 users라는 테이블 스페이스가 자동으로 생성된다.
*/
--테이블 스페이스 조회하기
...
alter user test_user1 quota 2m on system;/*
            test_user1이 system 테이블스페이스에 테이블을 생성
            2m의 용량을 할당한다.
        */
...
--테이블생성시도1 => insufficient privileges:권한불충분
--테이블생성권한 추가
grant create table to test_user2;
--테이블생성시도2 => no privileges on tablespace 'USERS':테이블스페이스에 권한없음
...
/*
생성된 사용자의 디폴트 테이블스페이스를 변경하고 싶은 경우
...
/*
# sqlplus에서 ED명령어
    : 긴 쿼리문을 작성할 때 메모장을 editer로 활용하여
    실행시키는 명령

1. 파일 생성하기
SQL> ed new_table [Enter]
-> 이와같이 했을때 사용자계정 디렉토리에 new_table.sql파일이 생성된다.

2. 메모장에서 테이블을 생성한다.
    마지막 문장에 /를 꼭 붙여준다.
    작성이 완료되면 저장후 파일을 닫아준다.
...
SQL> ed 파일명 [Enter]
즉, 해당 파일이 있으면 수정이고, 없다면 새롭게 생성하게된다.
*/        
