...
*/
--2017년1월1일부터 지금까지 지난 개월수는 얼마인가요?
select
    months_between(sysdate, '1998-02-26') "슬기는 몇개월?",
    months_between(sysdate, to_date('1995년07월11일', 'yyyy"년"mm"월"dd"일"')) "결과2", 
    round(months_between(sysdate, '1995-7-11')) "결과3"
from dual;
--만약 날짜 서식이 아니라면 to_date()로 날짜형식을 변경해준다.
...
select
    first_name, last_name, hire_date,
    trunc(months_between(to_date('2017년12월31일', 'yyyy"년"mm"월"dd"일"'), hire_date), 2) "근속개월수1",
    trunc(months_between('2017-12-31', hire_date), 2) "근속개월수2"
from employees
order by "근속개월수1" asc;
/*
...
add_months() : 날짜에 개월수를 더한 결과를 반환한다.
    형식] add_months(현재날짜, 더 할 개월 수)
*/
--현재를 기준으로 7개월 이후 날짜를 구하시오.
select
    to_char(sysdate, 'yyyy-mm-dd') "현재날짜",
    to_char(add_months(sysdate, 7), 'yyyy-mm-dd') "7개월이후 날짜"
from dual;
