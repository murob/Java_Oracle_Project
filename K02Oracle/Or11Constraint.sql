...
/*
    ...
    4번과 6번은 동일한 데이터로 인식되어 에러가 발생된다. 
*/
...
insert into tb_primary4 values ('kosmo', '코스모');--항상 부모테이블에 먼저 입력후
insert into tb_foreign4 values (1, '1번입니다', 'kosmo');--자식 테이블에 입력해야한다.
...
delete from tb_primary4;/*
             부모테이블에서 레코드 삭제시 on delete cascade 옵션때문에
...
create table tb_not_null (
    ...
    m_pw varchar2(40) null,        --null허용(일반적으로 null은 생략한다)
    m_name varchar2(50)            --null허용(이와같이 선언한다)
);
desc tb_not_null;
...
insert into tb_not_null values (40, '', '4444', '사길동');--입력실패. null을 입력할수없음
insert into tb_not_null (m_id, m_pw, m_name)
    values ('hong5', '5555', '오길동');--입력실패. m_idx컬럼에 값이 입력되야함.
...
insert into tb_default values ('bbbb', '');--null값이 입력됨
...
insert into tb_check2 values (6);--입력실패. check제약조건 위배
