...
-프로시저는 reture문이 없는 대신 out파라미터를 통해 값을 반환한다.
-본안성을 높일 수 있고 네트워크의 부하를 줄일 수 있다.
형식]
...
*/
--in파라미터를 생성한다. 사원테이블의 사원명을 참조하는 참조변수형태.
...
--프로시저 시행시 각각의 파라미터를 전달한다. 바인드 변수는 :을 붙여야한다.
...
쿼리의 일부로 사용되므로 반드시 return값이 있어야한다.
*/
...
is
    ...
    returnVal varchar2(10);--성별 확인후 남자 or 여자를 저장
begin
    --주민번호 자르기
...
select findGender('999999-1000000') from dual; -- -> 남자반환됨
select findGender('999999-4000000') from dual;
*/
...
/
insert into trigger_dept_original values (300, '열정의방', 10, 101);
...
update trigger_dept_original
    set department_name='진실의방', manager_id=78
...
begin
    --Or03에 사용예제있음. 문자열의 오른쪽을 특수기호로 채워준다.
...
execute KosmoMemberInsert('pro01', '1234', '프로시저', :i_result);
print i_result;
select * from member;
...
execute KosmoMemberInsert('pro1','1234','프로시저',:insert_count);
print insert_count;
...
/*
시나리오] member테이블에서레코드를삭제하는 프로시저를생성하시오.
    매개변수 : In -> member_id(아이디)
                Out ->returnVal(성공/실패반환)
...
--둘다일치, 아이디만 일치, 둘다틀린 3가지 케이스를 모두 테스트해볼것
