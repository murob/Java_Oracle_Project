...
/*
부서별 급여의 합계는 distinct를 사용해서 SQL문을 작성할수 없다.
*/
...
/*
count(*)와 같이 그룹과 관련된 조건은 where절에 사용할수없다.
*/
select
    department_id "부서번호", job_id "담당업무ID", 
    count(*) "사원수", avg(salary) "평균급여"
...
group by job_id
--order by count(*) asc;
order by ClerkCount asc;
/*
    물리적으로 존재하지 않는 컬럼의 경우 order by 절에
    사용할 때는 원형을 그대로 쓰거나, 별칭을 사용하면 된다.
*/
...
where salary>=10000 
group by job_id;
--순수하게 급여가 얼마 이상 = 물리적으로 존재하는 것임으로 where절에 써야함
--급여의 최소, 급여의 합 등 = 물리적으로 존재하지 않는 것임으로 having절에 써야함
...
관리자를 알수없는 사원 및 최저급여가 3000미만인 그룹은 제외시키고 
...
select
    ...
    min(salary)
from employees
where manager_id is not null --물리적으로 존재하는 컬럼만 쓸 수 잇음
group by job_id
having min(salary)>3000 --물리적으로 존재하지 않는 컬럼만 가능하면 group by에 의해 분류된 값들을 비교함
--having not min(salary)<3000
order by min(salary) desc;
/*
    관리자를 알 수 없는 => 물리적으로 존재하는 컬럼이므로 where에 기술
...
모든 사원의 평균급여를 출력하시오. 평균급여는 정수로 반올림하고 세자리마다 컴마를 출력하시오.
decode함수를 사용하여 각 부서번호에 맞는 부서명과 지역명을 나오게 하시오.
*/
select
    department_id "부서번호",
