...
declare
begin
    --PL/SQL에서 사용할때는 반드시 :을 변수앞에 붙여야한다. 
...
/
--위 예제를 치환연산자를 통해 숫자를 입력받은후 판단할수 있도록 수정
declare
    --치환연산자는 선언부에 사용가능함
    num number := &num;
begin
...
begin
    ...
    elsif로 사용해야한다. 또한 중괄호 대신 then과 ;이 사용된다.
    */
    if emp_dept = 50 then
        dept_name := 'Shipping';
...
반복문1 : Basic loop문
    java의  do~while문과 같이 조건체크없이 일단 loop로 진입한후
    탈출조건이 될때까지 반복한다. 
*/
declare
    num number := 0; 
...
/
/*
퀴즈] Basic loop문으로 1부터 10까지의 정수의 합을 구하는 
    프로그램을 작성하시오. 
*/
declare
    i number := 1;--증가시킬 변수
    ...
    /*
    변수명으로 sum을 사용할수 없다. 예약어 이므로 오류가 발생한다. 
    */
begin
    /*
...
/*
퀴즈] while loop문으로 1부터 10까지의 정수의 합을 구하는 
    프로그램을 작성하시오. 
*/
declare
    i number := 1;
...
/*
반복문3 : for문
    반복의 횟수를 지정하여 사용할수 있는 반복문으로, 반복을 위한 
...
/*
컬렉션(배열)
    일반 프로그래밍 언어에서 사용하는 배열타입을 PL/SQL에서는 테이블 타입이라고
    한다. 1차원, 2차원 배열을 생각해보면 테이블(표)와 같은 형태이기 때문이다.
...
--문제의 조건을 통한 쿼리에서 다수행이 인출되므로 Cursor를 사용해야한다.
declare
    --쿼리문을 통해 커서 생성
    cursor emp_cur is
...
: 고정길이를 가진 배열로서 일반 프로그래밍 언어에서 사용하는 배열과
    동일하다. 크기에 제한이 있어서 선언할때 크기(요소의갯수)지정하면
    이보다 큰 배열로 만들 수 없다.
    형식]
        Type 배열타입명 Is 
...
begin
    ...
    nt_array(4) := '네번째값할당';
    dbms_output.put_line(nt_array(4));--4번째값까지 할당 및 출력가능
...
declare    
    ...
    중첩테이블 자료형 선언 및 변수선언 : 사원테이블 전체 컬럼을 참조하는
        참조변수의 형태이므로 하나의 레코드(Row)를 저장할수 있는
        형태로 선언하였다. 
    */
    type ntType is 
...
begin
    --선언한 변수를 통해 select절을 수정한.다
    select ename, length(ename) 
        into emp_name, name_length
    from emp where empno = &input_id;
...
begin
    for j in 1 .. 5 loop
        if j = 1 then
            ...
            dbms_output.put_line('01000');
        elsif j = 3 then
            dbms_output.put_line('00100');
        elsif j = 4 then
            dbms_output.put_line('00010');
        elsif j = 5 then
            dbms_output.put_line('00001');
        end if;
    end loop;
end;
/                                               
