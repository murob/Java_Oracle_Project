...
where절을 이용해서 조건에 맞는 레코드 추출하기
    ...
    싱글쿼테이션을 사용해야 한다. 숫자형인 경우 생략 가능하다.
*/
...
/*
distinct : 칼럼에서 중복되는 레코드를 제거할 때 사용한다.
...
like 연산자 : 특정 키워드를 통한 문자열 검색하기
    ...
    Ex) D로 시작하는 3글자의 단어 : D__ -> Dab, Ddd, Dxy
        A가 중각에 들어가는 3글자의 단어 : _A_ -> aAa, xAy
*/
...
/*
레코드 정렬하기(Sorting)
    오름차순정렬 : order by 컬럼명 asc 9혹은 생략가능[디폴트])
