...
sysdate : 현재날짜와 시간을 초단위로 반환해준다. 주로 게시판에서
    새로운 포스팅이 있을 때 입력한 날짜를 표현하기 위해 사용된다.
*/
...
select to_char(sysdate, '오늘은 yyyy년 mm월 dd일 dy요일입니다') "과연될까?" from dual;
select to_char(sysdate, '"오늘은" yyyy"년" mm"월" dd"일" dy"요일입니다"') "과연될까?" 
...
from dual;--오라클의 모든 함수는 2개이상 중첩해서 사용할 수 있다.

select
    to_char(12345, '000,000'),
    to_char(12345, '999,999'),
    to_char(12345, '999,000'),
    ltrim(to_char(12345, '999,999,000')) "좌측공백제거2",
    ltrim(to_char(12345678, '999,999,000')) "좌측공백제거2"
from dual;
...
*/
select to_date('31-03-2021', 'dd-mm-yyyy') "날짜서식알려주기1" from dual;
select to_date('03-31-2021', 'mm-dd-yyyy') "날짜서식알려주기2" from dual;
...
[퀴즈] 문자열 '2013년10월24일'은 어떤 요일인지 변환함수를 통해
    출력할 수 있는 쿼리문을 작성하시오. 단, 문자열은 임의로 변경할 수 없습니닫.
*/
select
    to_date('2013년10월24일', 'yyyy"년"mm"월"dd"일"') "1단계:날짜로변경",
    to_char(to_date('2013년10월24일', 'yyyy"년"mm"월"dd"일"'), 'day') "2단계:요일로변경"
from dual;


/*
[퀴즈] : hr계정의 employees 테이블에서 사원번호 206인 사원이 어떤요일에
...
select
    ...
    to_char(to_date('2015-10-24 12:34:56', 'yyyy-mm-dd hh:mi:ss'), 'yyyy"년"mm"월"dd"일"dy"요일"') "2단계:결과출력"
from dual;
...
select
    first_name, commission_pct, nvl(commission_pct, 0) AS
...
/*
case() : java의 if~else문과 비슷한 역할을 한느 함수
    형식] case
            when 조건1 then 값1
            when 조건2 then 값2
            .....
