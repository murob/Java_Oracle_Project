...
형태로 기술해야 한다.
-테이블의 별칭을 사용한다면 "별칭명.컬럼명" 형태로 쓸 수도 있다.
*/
...
from employees inner join departments
    on employees.department_id=departments.department_id;/*
        에러발생됨. 양쪽 테이블 모두에 존재하는 department_id컬럼이
        있으므로 'column ambiguously defined' 에러가 발생한다.
        이때는 어던 테이블에서 가져올지 명시해야 한다.
    */

--as(별칭) 없이 작성하여 쿼리문이 길어짐. 테이블명으로 그대로 사용함.
select
    employee_id,
    first_name,
    last_name,
...
select
    ...
    email, emp.department_id, department_name
...
select
    ...
    department_name, J.job_id, job_title, city
...
outer join을 사용할 때는 반드시 outer전에 데이터를 어떤 테이블을 기준으로
가져올지를 기술해야한다.
...
형식2(오라클방식)
    ...
    where 
        테이블1.기본키=테이블2.외래키(+)
        and 조건1 or 조건2 ... 조건n;
    => 오라클 방식으로 변경시에는 outer join 연산자인 (+)를 where에 붙여준다.
...
select
    ...
    department_name, Loc.location_id, city, state_province
...
시나리오] 사원테이블에서 각 사원의 매니저아이디와 매니저이름을 출력하시오.
    ...
    2.사원의 매니저아이디와 매니저의 사원아이디를 join한다.
...
from
    departments
where
    departments.department_id=employees.first_name
