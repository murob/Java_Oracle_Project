...
initcap(문자열)
    ...
    -알파벳과 숫자를 제외한 나머지 문자 다음에 나오는
        첫번째 문자를 대문자로 변환한다.
*/
select initcap('hi gello 안녕') from dual;
...
/*
lpad(), rpad()
    : 문자열의 왼쪽, 오른쪽에 특정한 기호로 채울대 사용하는 함수
...
select substr('good morning john', 8) from dual;--ring john
...
--trim은 좌우측 공백은 제거되나, 중간의 공백은 제거되지 않는다.
...
instr() : 해당 문자열에서 특정문자가 위치한 인덱스값을 반환한다.
    형식1] instr(컬럼명, '찾을문자')
        => 문자열의 처음부터 문자를 찾는다.
    형식2] instr(컬러명, '찾을문자', '탐색을 시작할 인덱스', '몇번째문자')
...
--10번째 인덱스부터 탐색을 시작하여 발견된 2번째 인덱스소환
